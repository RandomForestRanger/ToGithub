#!/usr/bin/env python3
"""Opdrag 8b: candidate generator + selector for Type 24 (K+R vs K+2 connected P), mate.
...
Tier construction (not pure DTM banding, per spec):
  bronze -> pawn pair on rank 4 or 5 (rook wins them cleanly, attacking from behind/side)
            ...
            for dbf, dbr in BK_OFFSETS:
                ...
                for dwf, dwr in WK_OFFSETS:
                    ...
                    for dqf, dqr in WQ_OFFSETS:
                        ...
                        board.set_piece_at(wq_sq, chess.Piece(chess.ROOK, chess.WHITE))
...
def main():
    ...
    try:
        for tier in ('gold', 'bronze', 'silver'):
            ...
            budget = TIER_BUDGET_FRAC[tier] * TIER_MOVE_LIMIT[tier]
            # Gold: hardest-first within its construction band (see the same
            # fix in generate_type23.py -- lowest-dtm-first silently picked
            # easier-than-bronze gold puzzles there).
