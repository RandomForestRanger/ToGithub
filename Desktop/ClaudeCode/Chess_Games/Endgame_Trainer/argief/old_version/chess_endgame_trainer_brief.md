...
All positions are hardcoded as FEN strings, stored in a `positions.js` data file grouped by type and tier.
...
Use standard algebraic notation but with Afrikaans piece letters:
- K = Koning (King)
- K = Koningin (Queen) — disambiguate visually with icons
- T = Toring (Rook)
- L = Loper (Bishop)
- R = Ruiter (Knight)

> Alternatively, use figurine algebraic notation (piece icons instead of letters) to avoid disambiguation issues. Recommended.

---

## First Sprint — Build This First

Before building all 200 positions and the full badge infrastructure, validate the entire game loop with:

1. **One fully playable puzzle:** Type 1 (Koning & Koningin teen Koning), Brons, Position 1
2. Stockfish WASM playing black at full strength
3. Hint system (*"Wenk"*) working
4. Stalemate detection with correct Afrikaans message
5. Move limit detection with correct Afrikaans message
6. Post-round replay functioning (perfect game, 2s/move, arrows)
7. Basic badge screen showing the single badge

Once the loop is validated end-to-end, scale to all 200 positions and full badge/progression infrastructure.
